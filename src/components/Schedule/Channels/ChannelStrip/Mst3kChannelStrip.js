...
const Mst3kChannelStrip = ({ name, schedule, timeDigit, ampm, passIdChann, sendPrograms }) => {
    ...
    if (ampm === "am") {
        thisHoursPrograms = firstFilter.filter(program => (program.ampm === "am" && program.hour !== 12) || (program.hour === 12 && program.ampm === "pm"))
    } else {
        thisHoursPrograms = firstFilter.filter(program => (program.ampm === "pm" && program.hour !== 12) || (program.hour === 12 && program.ampm === "am"))
    }
